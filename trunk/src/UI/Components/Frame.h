...
namespace Components {
	...
	public:
		...
		virtual void SetPosition(const Utils::Vector2 &vPosition) {
			...
			OnSetPositionEvent(vPosition);
		}

		// When it handled the most recent event
		Utils::Event<void ()> OnFocusStartEvent;
		Utils::Event<void ()> OnFocusStopEvent;
		Utils::Event<void (const Utils::Vector2&)> OnSetPositionEvent;
